...
static int
r=0,indicate=0,Sts=0,Status=0,Error=0,Confirmed=0,Fix=0,d=0,Check=0,transmit=0,rcve=0,receive=0,Step=0,coach1=0,send=0,S=0,time=0,Alert=0;
int e=3909;
#define T1 22
#define T2 23
#define T3 24
...
void loop()
{
if(digitalRead(ALRTIN)==1) /* This signal comes from other platforms so that this platform will receive Train number for
...
{
x=HC12.read(); /* ....x receives HC12 value. */
EEPROM.write(1,x);
...
Fix==1; /* Fix becomes 1 so that train number will be received by d from x and checked whether equal to Tno. */
EEPROM.write(3,Fix);
digitalWrite(ALRTOUT,HIGH); /* Then this station sends an Alert signal to other platforms. */
...
{
d=x*(10^-8); /* Checked whether equal to Tno so that only extraction takes place until train departs. */
EEPROM.write(4,d);
}
else
{
...
else if((104300000<=x<=104312999)&&d==Tno) /* if 4th digit from LHS contains 3, it indicates power interruption with next
compartment which does not receives details from this compartment that sends x value to the station. */
{
counter=(x-104300000);
EEPROM.write(5,counter);
z==4;
...
if(((100100000<x<=104100999)||(104112999<x<=104300999)||(210100000<x<=210100999)||(210112999<x<=210300999))&&d==Tno
) /* If the upcoming train sends a wrong station code……... */
{
...
else if(((100300000<x<=104300999)||(104312999<x<=210300999)||(210312999<x))&&d==Tno) /* Same condition but the platform
number is 3. */
{
Check=x*(10^-5);
EEPROM.write(10,Check);
...
{
P121==0;
/* This indicates priority which is needed so that the compartment details which are transmitted already should not be
repeated again and again leaving the other compartment details which would have been unable to transmit due to
environmental conditions or communication interferences. */
EEPROM.write(12,P121);
y=counter-1000;
EEPROM.write(11,y);
Wire.beginTransmission(1);
...
if(Sts==104111||Sts==211111||Sts==104311||Sts==211311) /* When all the values as last digit 1, it indicates Arrival shown by A
which is 1st letter. */
{
...
if(P121==0&&P112==0&& P103==0&& P94==0&& P85==0&& P76==0&& P67==0&& P58==0&& P49==0&& P310==0&&
P211==0&& P1P12==0)
...
{
/* If any Priority value remains 1 even though the train has arrived as shown by Status value 1, then Arduino at station sends e=
3909 to Arduino which displays the error compartment. 3909 indicates Error as shown by inverting 3 to get E letter.*/
if((P121==1||P112==1||P103==1||P94==1||P85==1||P76==1||P67==1||P58==1||P49==1||P310==1||P211==1||P1P12==1)
||(Status==1))
{
if(P121==1)
{
...
if(TD<10&&Status==1) /* Then the station checks whether LDR (TD) placed in between the platform track has value less than
10 and Status value is 1 indicating that the train arrived so that Laser from station sends light once to 1st compartment to receive
the Train number during Step 1. At Step 2 it transmits to the last compartment so that other compartments which are not able to
receive the details from the 1st compartment will acquire details from the last compartment. */
{
...
if(Step==2&&(Check==0||Check==1)) /* Check value becomes zero after extraction of train number if station code received from
the upcoming train is wrong. */
{
...
/* "z" means station code for forward journey. After the train departs, it sends "Sts" value with last digit as 4 indicating
Departure where D is the 4th letter. Then it crosses first over TD3 sensor and then TD4. After crossing both, train details are
erased and all the values such as Status, Alert, Confirm, r and indicate values becomes zero. */
...
else if((z==11||r==1)&&TD2<10&&Status==4) /* This loop is like previous loop which is used for return journey as shown by
z=11. */
{
...
else if(z==4&&TD2<10&&TD1>10) /* Before arriving at station, the train moves over TD2 and TD1. After moving at both
sensors, high intensity will make return 'r' value as zero so that if that train will make its return journey, then this r value will
be useful. */
{
digitalWrite(T2,HIGH);
digitalWrite(T1,LOW);
...
else if(z==11&&TD4<10&&TD3>10) /* Same followed for return journey trains which may go towards previous station in case
